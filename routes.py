...
def create_account():
	...
	if request.method == "POST":
		username = request.form["username"]
		exists = users.check_if_already_user(username)
		if exists:
...
def profile(id):
	if request.method == "GET":
		...
		if allow:
			...
			return render_template("profile.html", students_courses=students_courses, admin =admin, courses_created = courses_created, user = user)
...
#		student_id = request.form["student_id"]
		...
		return render_template("students_attending_course.html", attending = attending, course_id = course_id)
...
def create_new_multiple_choice_question():
		if session["csrf_token"] != request.form["csrf_token"]:
			...
			for choice in task_choices:
				if choice != "":
					choices.add_choice(task_id, choice)
...
def add_answer():
	if request.method == "POST":
		if session["csrf_token"] != request.form["csrf_token"]:
			...
			course_id = request.form["course_id"]
			answers.add_answer(task_id, student_id, content=content, course_id=course_id)
			return redirect("/courses/" + str(course_id))
...
def student_answers():
	if session["csrf_token"] != request.form["csrf_token"]:
		...
		course_id = request.form["course_id"]
		course = modify_courses.get_course(course_id)
		task = modify_tasks.get_task(task_id)
		all_answers = False
