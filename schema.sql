...
CREATE TABLE answers (id SERIAL PRIMARY KEY, task_id INTEGER REFERENCES tasks ON DELETE CASCADE, task_topic TEXT, student_id INTEGER REFERENCES users ON DELETE CASCADE, sent_at TIMESTAMP, content TEXT, course_id INTEGER REFERENCES courses ON DELETE CASCADE);
...
CREATE TABLE choices (id SERIAL PRIMARY KEY, choice_text TEXT, task_id INTEGER REFERENCES tasks ON DELETE CASCADE, task_topic TEXT);
